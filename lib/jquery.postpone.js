...
;(function($){
  ...
  var
    ...
    //A map of the m used to multiply a value when suffixed with certain time-unit-abbreviations.
    m = {
      ms: 1,
      cs: 10,
      ds: 100,
  ...
  function postpone(args, options){
    ...
    var setter = (options.type == 1 ? setInterval : setTimeout);
    
    //Define the unsetter we are using.
    var unsetter = (options.type == 1 ? clearInterval : clearTimeout);
    
    //Define the boolean that will keep track of wether the timeout is set.
    var isset = false;
    
    //Define some variable that we may make use of in the callbacks.
    var i=0, autoresolve=-1;
    
    //Define the callback.
    var callback = (function(){switch(options.type){
    ...
    $.extend(P, {
      ...
      //Postpone a running timeout.
      postpone: function(){},
      
      //Auto complete the "every" or "recur" after n progress calls.
      times: function(n){
        i=0;
